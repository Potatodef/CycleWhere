import { corridorSeeds } from "../data/corridors";
import { anchorSeeds } from "../data/anchors";
import { average, classifyFairness, majorityFriendlySpread, spread, standardDeviation } from "./fairness";
import { clamp, haversineKm, offsetPerpendicularKm, polylineDistanceKm } from "./geo";
import { estimateTransitMinutesBetween } from "./transit";
function buildGeometry(start, corridor, detour) {
    const points = [
        start,
        ...detour.controlPoints.map((control) => offsetPerpendicularKm(start, corridor.endpoint, control.t, control.perpendicularKm)),
        corridor.endpoint
    ];
    return points;
}
...
function getPreferredAnchorPoint(corridor) {
    return (anchorSeeds.find((anchor) => anchor.id === corridor.preferredAnchorId)?.point ??
        corridor.endpoint);
}
...
export function buildTransitQueries({ start, participants, startTimeIso }) {
    const queries = [];
    for (const corridor of corridorSeeds) {
        for (const detour of corridor.detours) {
            ...
            const endpointForTransit = haversineKm(preferredAnchor, corridor.endpoint) < 1
                ? corridor.endpoint
                : preferredAnchor;
    ...
    return corridorSeeds
        ...
        const participantTimes = participants.map((participant) => {
            ...
            const transitMinutes = transitOverrides?.[routeTransitKey(routeId, participant.id)] ??
                estimateTransitMinutesBetween(endpointForTransit, participant.anchor.point, departureIso, participant.anchor.kind);
            return {
        ...
        const cyclingPathCoverage = clamp(corridor.baseCyclingPathCoverage + (detour.distanceMultiplier - 1) * 0.08, 0.04, 0.34);
        const commonCorridorCoverage = clamp(corridor.baseCommonCorridorCoverage + (detour.distanceMultiplier - 1) * 0.12, 0.3, 0.92);
